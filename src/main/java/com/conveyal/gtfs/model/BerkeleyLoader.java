...
public abstract class BerkeleyLoader<E extends Entity> {

    private static final Logger LOG = LoggerFactory.getLogger(Entity.Loader.class);
    private static final Deduplicator deduplicator = new Deduplicator();

    protected final BerkeleyFeed feed;    // the feed into which we are loading the entities
    ...
    public BerkeleyLoader (BerkeleyFeed feed, String tableName) {
        this.feed = feed;
     ...
     */
    protected <V> V getRefField(String column, boolean required, PrimaryIndex<String, V> target) throws IOException {
        String str = getFieldCheckRequired(column, required);
        if (str != null) {
        ...
        return null; // not storing references anymore
    }
